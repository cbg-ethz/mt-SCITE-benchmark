...
def simulate_read_counts(mutation_tree, error_rate, num_reads, num_cells, n_err_mutations, n_err_samples):
    ...
    """
    Simulates read counts for mitochondrial mutations across cells.

This function generates read counts for clonal and non-clonal (erroneous) mitochondrial mutations, considering heteroplasmy and sequencing errors. Clonal mutations are derived from a mutation tree, while non-clonal mutations are assigned randomly to cells.

Parameters:
    mutation_tree (networkx.Graph): Mutation tree where each node contains 'mutation_freq' (list of mutation frequencies).
    error_rate (float): Probability of sequencing errors.
    num_reads (int): Number of sequencing reads per site.
    num_cells (int): Total number of cells to simulate.
    n_err_mutations (int): Number of non-clonal (erroneous) mutations.
    n_err_samples (int): Number of cells affected by each erroneous mutation.

Returns:
    list: 
        - read_counts (numpy.ndarray): Shape (num_cells, n_mutations + n_err_mutations, 4). Simulated read counts for each mutation and cell.
        - mutation_probabilities (numpy.ndarray): Shape (num_cells, n_mutations). Mutation frequencies (heteroplasmy) for clonal mutations.
    """

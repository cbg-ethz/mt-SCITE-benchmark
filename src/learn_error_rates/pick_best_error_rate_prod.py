...
def compute_normalized_likelihood(dat, dat_star, epsilon=1e-10):
    ...
    for row in range(1, normalised_dat.shape[0]):
        ...
        normalised_dat.iloc[row, 1:] = dat.iloc[row, 1:] * (
...
def main():
    ...
    new_tree_file = os.path.join(output_dir, f"best_learned_tree_6.gv")
    new_error_rate_file = os.path.join(output_dir, "best_error_rate_6.csv")

...
compute_normalized_likelihood <- function(dat, dat_star) {
  ...
  for (row in 2:nrow(dat)) {
    min_tree_log_lik = min(dat[row, -1])
    threshold = 10 *min_tree_log_lik
    bounded_log_star = pmax(threshold, dat_star[row, -1])
    normalised_dat[row, -1] <- dat[row, -1] - bounded_log_star
  }
  return(normalised_dat)
}
...
#new_dat_2 = compute_normalized_likelihood_2(dat, dat_star)
...
g= ggplot(dat_to_plot, aes(x=log(error_rate), y=mean_log_lik)) + 
  ...
  ylab("Normalised tree likelihood")+
  xlab("Log error rate")
...
ggsave(plot = g, width = 6.4, height = 4.8, units = "in",
       filename = paste0("plots/", n_mutations,"_120_", format(true_error_rate, scientific = F),
                         "_500_", n_cells, "_", initial_mut_freq, "_", seed, "_normalised_log_lik.png"))
